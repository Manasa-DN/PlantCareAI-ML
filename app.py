...
try:
    ...
    raise ImportError(
        "Failed to import Keras/TensorFlow.\n\n"
        "Install the project dependencies first:\n"
        "  .\\vev\\Scripts\\python.exe -m pip install -r requirements.txt\n\n"
        "Then run the app with:\n"
        "  .\\vev\\Scripts\\python.exe app.py\n\n"
...
MODEL_ARCHIVE_PATH = Path("plant_disease_mobilenet_v2.keras")
MODEL_SAVEDMODEL_PATH = Path("plant_model")


def _build_saved_model_wrapper(model_dir: Path):
    if not hasattr(keras.layers, "TFSMLayer"):
        raise RuntimeError("Installed Keras version does not support loading SavedModel via TFSMLayer.")

    inputs = keras.Input(shape=(160, 160, 3), name="input_image")
    outputs = keras.layers.TFSMLayer(str(model_dir), call_endpoint="serving_default")(inputs)
    if isinstance(outputs, dict):
        outputs = next(iter(outputs.values()))
    return keras.Model(inputs=inputs, outputs=outputs, name="plant_disease_saved_model")
...
def load_prediction_model():
    ...
    if MODEL_ARCHIVE_PATH.exists():
        try:
            return keras.models.load_model(MODEL_ARCHIVE_PATH)
        except Exception as exc:
            errors.append(f"{MODEL_ARCHIVE_PATH.name}: {exc}")

    if MODEL_SAVEDMODEL_PATH.exists():
        try:
            return keras.models.load_model(MODEL_SAVEDMODEL_PATH)
        except Exception as exc:
            errors.append(f"{MODEL_SAVEDMODEL_PATH.name}: {exc}")

    joined = "\n - ".join(errors) if errors else "No compatible model files were found."
    raise RuntimeError(
        "Could not load the plant disease model.\n"
        "Tried these model sources:\n"
        f" - {joined}\n\n"
        "Use the dependencies from requirements.txt so Keras 3 can read this model."
    )
...
def generate_gradcam(image_path, model, predicted_class_idx, target_size=(160, 160)):
    img = Image.open(image_path).convert('RGB')
    img_array = img_to_array(img.resize(target_size))
...
# Derive class names based on model output size
num_classes = model.output_shape[-1]
if num_classes == len(PLANTVILLAGE_CLASS_NAMES):
...
def _find_gradcam_target_layer(model_obj):
...
def generate_gradcam_assets(model_obj, img_array, original_image_path: str):
    target_layer = _find_gradcam_target_layer(model_obj)
    if target_layer is None:
        return None

    grad_model = keras.models.Model(
        inputs=model_obj.inputs,
        outputs=[target_layer.output, model_obj.output],
    )

    input_tensor = keras.ops.convert_to_tensor(img_array)
    with tf.GradientTape() as tape:
        ...
        pred_index = int(np.argmax(predictions_np[0]))
    ...
    heatmap = keras.ops.sum(conv_outputs * pooled_grads, axis=-1)
    heatmap = keras.ops.maximum(heatmap, 0)
    max_value = float(np.max(np.array(heatmap)))
    if max_value <= 0:
        return None

    heatmap = np.array(heatmap) / max_value

    base_image = Image.open(original_image_path).convert("RGB")
    heatmap_uint8 = np.uint8(np.clip(heatmap * 255, 0, 255))
    heatmap_img = Image.fromarray(heatmap_uint8, mode="L").resize(base_image.size, Image.Resampling.BILINEAR)
    heatmap_arr = np.asarray(heatmap_img).astype("float32") / 255.0

    red = np.clip(255 * heatmap_arr * 1.25, 0, 255)
    green = np.clip(200 * heatmap_arr * 0.85, 0, 255)
    blue = np.clip(70 * (1.0 - heatmap_arr), 0, 255)
    color_map = np.stack([red, green, blue], axis=-1).astype("uint8")
    overlay = Image.blend(base_image, Image.fromarray(color_map), alpha=0.42)

    stem = Path(original_image_path).stem
    overlay_path = GRADCAM_DIR / f"{stem}_gradcam.jpg"
    heatmap_path = GRADCAM_DIR / f"{stem}_heatmap.jpg"
    overlay.save(overlay_path, quality=92)
    Image.fromarray(color_map).save(heatmap_path, quality=92)
...
def build_gradcam_context(model_obj, img_array, original_image_path: str):
    assets = generate_gradcam_assets(model_obj, img_array, original_image_path)
    if assets:
        return {
            "gradcam_path": assets["overlay_path"],
            "gradcam_overlay_path": assets["overlay_path"],
            "gradcam_heatmap_path": assets["heatmap_path"],
            "gradcam_layer": assets["target_layer"],
        }

    legacy_path = generate_gradcam(original_image_path, model_obj, int(np.argmax(model_obj.predict(img_array, verbose=0))))
    return {
        "gradcam_path": legacy_path,
        "gradcam_overlay_path": legacy_path,
...
def upload():
    if request.method == "POST":
        ...
        if file and file.filename:
            ...
            img = preprocess_image(str(filepath))
            prediction = model.predict(img, verbose=0)
            confidence = float(round(float(np.max(prediction)) * 100, 2))
            predicted_class_idx = np.argmax(prediction)
            predicted_class = class_names[predicted_class_idx]

            gradcam_context = build_gradcam_context(model, img, str(filepath))
